...
{
	if (argc != 4)
		error("USE: ./main path/to/template n_iters padding", 20);
...
board *get_from_file(struct parameters *the_parameters)
{
	...
	if (strcmp(file_extension, "json") == 0)
		return get_from_json(the_parameters->template_clue, the_parameters->padding);
	if (strcmp(file_extension, "rle") == 0)
		return get_from_rle(the_parameters->template_clue, the_parameters->padding);
	if (strcmp(file_extension, "cells") == 0)
		return get_from_plaintext(the_parameters->template_clue, the_parameters->padding);
